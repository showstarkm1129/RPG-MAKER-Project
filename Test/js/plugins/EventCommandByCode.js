 ...
 *
 * 可通过直接指定代码与参数来执行事件指令。
 * 可在各指令的参数中使用变量（控制字符，见注释*1），
 * 也可以强制指定通常事件指令无法设置的超出范围的值。
 * 但若以非预期的参数调用指令，无法保证一定能正常运行。
 *
 * *1
 * \v[n] ：变量
 * \s[n] ：开关
 * \ss[n] ：自开关
 * \js[n] ：脚本（仅在应用 TextScriptBase.js 时可用）
 *
 * 无法指定在事件编辑器中会跨多行的指令，或深度嵌套的指令。
 *
 * 具体的参数指定方法可参考：
 * https://docs.google.com/spreadsheets/d/1aqY-xzFqT0vnZE-OkfsMYsP9Ud91vWTrBLU-uDkJ-Ls/edit#gid=2095105278
 */
